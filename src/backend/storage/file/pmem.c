...
int
PmemFileOpen(const char *pathname, int flags, size_t fsize, void **addr,size_t *mapped_len)
{
	return PmemFileOpenPerm(pathname, flags, PG_FILE_MODE_DEFAULT, fsize, addr, mapped_len);
}
...
void 
PmemFileFlush(void *map_addr, size_t len)
{
	return pmem_flush(map_addr,len);
}
