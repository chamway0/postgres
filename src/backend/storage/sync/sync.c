...
/*
通过遍历BufferDesc来合并需要sync的文件
*/
void AbsorbSyncRequests2(int flags)
{
	uint32		buf_state;
	int			buf_id;
	int			mask = BM_DIRTY;
	...
	for (buf_id=NBuffers+1; buf_id < NBuffers+1+totalAllocPmemBlocks; buf_id++)
	{
		...
		{
			recptr = BufferGetLSN(reln,bufHdr);
		...
		//即将要更新页面的校验和，增加引用计数
		//PinBuffer_Locked2(BufferDescriptorGetBuffer(bufHdr),buf_state);
		UnlockBufHdr(bufHdr,buf_state);

		TRACE_POSTGRESQL_BUFFER_FLUSH_START(bufHdr->tag.forkNum,
											bufHdr->tag.blockNum,
											bufHdr->smgr_rnode.node.spcNode,
											bufHdr->smgr_rnode.node.dbNode,
											bufHdr->smgr_rnode.node.relNode);
		...
		*/
		...
		// */
		// bufBlock = BufHdrGetBlock(NULL,bufHdr);

		
		// LWLockAcquire(BufferDescriptorGetContentLock(bufHdr), LW_EXCLUSIVE);

		// /*
		// 1.原有逻辑是加Buffer共享锁，加IO排他锁更更改状态位为IO_ON_PROGRESS，
		// 然后在页面拷贝上计算校验和（在这个过程中页面允许有新的写入），再写入文件中。
		// 2.因为去除了IO锁，所以这里要加Buffer排他锁，直接原地计算校验和
		// */
		// PageSetChecksumInplace((Page) bufBlock, bufHdr->tag.blockNum);

		// LWLockRelease(BufferDescriptorGetContentLock(bufHdr));

		// UnpinBuffer2(buf_id, true);


		/*
		将所有脏页按FileTag为Key放入散列表以合并相同文件的页，得到需要进行Flush的文件
		这里不需要加锁，只有checkpoint进程会进行此操作
		*/
		ftag.handler = SYNC_HANDLER_MD;
		ftag.rnode = bufHdr->tag.rnode;
		ftag.forknum = bufHdr->tag.forkNum;
		ftag.segno = bufHdr->tag.blockNum / ((BlockNumber) RELSEG_SIZE);

		oldcxt = MemoryContextSwitchTo(pendingOpsCxt);
		...
		TRACE_POSTGRESQL_BUFFER_FLUSH_DONE(	bufHdr->tag.forkNum,
											bufHdr->tag.blockNum,
											bufHdr->smgr_rnode.node.spcNode,
											bufHdr->smgr_rnode.node.dbNode,
											bufHdr->smgr_rnode.node.relNode);
